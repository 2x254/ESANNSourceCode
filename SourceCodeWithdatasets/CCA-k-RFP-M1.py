...
def conceptual_clustering(transaction_data, patterns, lamda,varr,k):
    ...
    for i in range(num_transactions):
        prob += lpSum(1*y[j] for j in range(num_patterns) if cover(transaction_data[i],patterns[j],k)) == varr, f"Transaction_Coverage_{i}"
    ...
    for j in range(num_patterns):
        if y[j].value() == 1:
            clusters.append([T  for T in transaction_data if cover(T,patterns[j],k)  ])
...
if __name__ == "__main__":
    ...
    path1='RelaxedPatterns/patternsk1.txt'
